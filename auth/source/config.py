...
class AWSEC2Config(ProductionConfig):
    """AWS ECS Fargate configuration with secure DB connection"""
    
    @property
    def SQLALCHEMY_DATABASE_URI(self):
        # Construct from individual environment variables (like ECS provides)
        ...
        if all([db_user, db_password, db_host, db_name]):
            ...
            return os.environ.get(
                'DATABASE_URL',
                'mysql+mysqldb://admin:root@orchestrator-db.cbumqc8u83v1.us-east-2.rds.amazonaws.com:3306/orchestrator-db'
            )
...
def get_config():
    ...
    env = os.environ.get('FLASK_ENV', 'aws')
    return config.get(env, config['aws'])